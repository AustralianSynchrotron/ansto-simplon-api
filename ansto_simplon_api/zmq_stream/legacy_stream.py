...
class LegacyStream:
    ...
    def _legacy_stream_start_message(self) -> None:
        ...
        omega = zmq_start_message.goniometer.get("omega", {})
        ...
        config_header = LegacyConfigHeader(
            ...
            countrate_correction_applied=zmq_start_message.countrate_correction_enabled,
            countrate_correction_count_cutoff=self.detector_config.detector_countrate_correction_cutoff,
            data_collection_date=str(zmq_start_message.arm_date),
            description=zmq_start_message.detector_description,
            detector_distance=float(zmq_start_message.detector_translation[2]),
            detector_number=zmq_start_message.detector_serial_number,
            detector_translation=zmq_start_message.detector_translation,
            eiger_fw_version=self.detector_config.eiger_fw_version,
            element="",
            flatfield_correction_applied=zmq_start_message.flatfield_enabled,
            frame_count_time=zmq_start_message.count_time,
            frame_period=zmq_start_message.frame_time,
            frame_time=zmq_start_message.frame_time,
            kappa_increment=0.0,
            kappa_start=0.0,
            nimages=int(self.number_of_frames_per_trigger),
            ntrigger=1,
            number_of_excluded_pixels=self.detector_config.detector_number_of_excluded_pixels,
            omega_increment=float(omega.get("increment", 0.0)),
            omega_start=float(omega.get("start", 0.0)),
            phi_increment=0.0,
            phi_start=0.0,
            photon_energy=zmq_start_message.incident_energy,
            ...
            pixel_size_x=zmq_start_message.pixel_size_x,
            pixel_size_y=zmq_start_message.pixel_size_y,
            roi_mode=self.detector_config.roi_mode,
            sensor_material=zmq_start_message.sensor_material,
            sensor_thickness=zmq_start_message.sensor_thickness,
            software_version=self.detector_config.software_version,
            detector_readout_time=self.detector_config.detector_readout_time,
            threshold_energy=float(zmq_start_message.threshold_energy["threshold_1"]),
            trigger_mode=self.detector_config.detector_trigger_mode,
            two_theta_increment=0.0,
            two_theta_start=0.0,
            virtual_pixel_correction_applied=zmq_start_message.virtual_pixel_interpolation_enabled,
            wavelength=zmq_start_message.incident_wavelength,
